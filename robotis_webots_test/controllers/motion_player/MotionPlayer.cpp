...
void MotionPlayer::run() {
  ...
  ofstream file("servo_value6.csv");
  ...
  Accelerometer *acc = Robot::internalGetInstance()->getAccelerometer("Accelerometer");
  acc->enable(mTimeStep);
  ...
  while (true){
    ...
    if (!motion.isOver()){
      ...
      file << gyro->getValues()[0] << ", "
           ...
           << acc->getValues()[1] << ", "
           << acc->getValues()[2] << ", "
           << Robot::internalGetInstance()->getTime() << "\n";
    } else {
      file.close();
      break;
    }
